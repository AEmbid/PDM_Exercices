...
public class MainActivity extends AppCompatActivity implements ItemFragment.OnListFragmentInteractionListener, ItemFragment.MiOnLongClickListener{
    ...
    @Override
    public void onMiLongClick(int id) {
        Bundle bundle=new Bundle();
        bundle.putInt(ID_SELECCIONADO, id);
        if (dosFragmentos){
            BotonFragment botonFragment=new BotonFragment();
            botonFragment.setArguments(bundle);
            getSupportFragmentManager().beginTransaction().replace(R.id.content_detalle,botonFragment).commit();
        }
        else{
